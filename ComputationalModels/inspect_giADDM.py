...
paramstofit = {'wself':1,'wother':1,'wfair':1,'bound':1,'collapse':1,'stbias':1, 'genbias':1, 'theta':1, 
    'dwself':1,'dwother':1,'dwfair':1,'dbound':1,'dcollapse':1,'dstbias':1, 'dgenbias':1, 'dtheta':1}

paramnames = ('wself','wother','wfair','bound','collapse','stbias', 'genbias', 'theta', 
    'dwself','dwother','dwfair','dbound','dcollapse','dstbias', 'dgenbias', 'dtheta')
...
for sub1 in np.arange(len(sub)):
    ...
    varthetadata = (chainlength-1)*W/chainlength + B/chainlength
    Rhat = np.sqrt(varthetadata/W)
...
#    
    ...
    # axs[1,3].plot(np.arange(len(theta)),theta)
    ...
    # axs[3,3].plot(np.arange(len(dtheta)),dtheta)
    ...
    #plt.hist(dtheta,100)
    sddtheta = np.nanstd(dtheta)
    
    
    
    fitpars = [mwself, mwother, mwfair,mbound,mcollapse,mstbias,mgenbias,mtheta, mdwself, mdwother, mdwfair, mdbound,mdcollapse,mdstbias,mdgenbias,mdtheta]
    fitparssd = [sdwself, sdwother, sdwfair,sdbound,sdcollapse,sdstbias,sdgenbias,sdtheta, sddwself, sddwother, sddwfair, sddbound,sddcollapse,sddstbias,sddgenbias,sddtheta]
    ...
    def iterate1(x): # x is a vector of iterables
        ...
        first = giADDM.get_giADDM_NLLs(i, pars1, nsimspfit, maxtime[0],selfamt1, otheramt1,finish1,fix1,choice1,noisematrix,precision)
        ...
        out2 = giADDM.get_giADDM_NLLs(i, pars2, nsimspfit, maxtime[1],selfamt2, otheramt2,finish2,fix2,choice2,noisematrix,precision)
    ...
    subjectfitpars[sub1,17] =Dthetabar
...
subjectfitpars.columns = ['subj','wself','wother','wfair','bound','collapse','stbias','genbias','theta','dwself','dwother','dwfair','dbound','dcollapse','dstbias','dgenbias','dtheta','Dthetabar','Dbar','DIC', 'acceptance']
...
dftranspars.columns = ['subj', 'wself','wother','wfair','bound','collapse','stbias','genbias','theta','dwself','dwother','dwfair','dbound','dcollapse','dstbias','dgenbias','dtheta']
